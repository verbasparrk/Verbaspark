...
export async function published(db,slug){if(!/^[a-z0-9][a-z0-9-]{2,29}$/.test(slug||''))throw Object.assign(Error('Page not found.'),{status:404});const row=checked(await db.from('published_pages').select('*').eq('slug',slug).maybeSingle());if(!row||row.moderated_at)throw Object.assign(Error('Page not found.'),{status:404});return row}
