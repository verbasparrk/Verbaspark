...
export function accountDialog(getState,loadState,saving,confirmPublish=async()=>true){
 ...
 const run=fn=>async event=>{event.preventDefault();const buttons=[...content.querySelectorAll('button')].map(button=>[button,button.disabled]);buttons.forEach(([button])=>button.disabled=true);try{await fn()}catch(e){message(e.code==='23505'?'That username is already taken.':e.message)}finally{buttons.forEach(([button,disabled])=>button.disabled=disabled)}};
