...
export async function saveDraft(document,{userId,revision,requestId}){
 ...
 for(const card of copy.cards.flatMap(c=>[c,...(c.images||[])])){
...
export async function resolveImages(document){for(const c of (document.cards||[]).flatMap(c=>[c,...(c.images||[])])){if(c.imagePath){const result=await cloud.storage.from('page-images').createSignedUrl(c.imagePath,3600);c.image=result.data?.signedUrl||''}if(c.file?.path){const result=await cloud.storage.from('page-files').createSignedUrl(c.file.path,3600);c.file.src=result.data?.signedUrl||''}}return document;}
export async function exportImages(document){const copy=structuredClone(document);for(const card of copy.cards.flatMap(c=>[c,...(c.images||[])])){
...
export function accountDialog(getState,loadState,saving,confirmPublish=async()=>true){
 ...
 const run=fn=>async event=>{event.preventDefault();const buttons=[...content.querySelectorAll('button')];buttons.forEach(b=>b.disabled=true);try{await fn()}catch(e){message(e.code==='23505'?'That username is already taken.':e.message)}finally{buttons.forEach(b=>b.disabled=false)}};
 ...
 if(row){content.querySelector('#publish-slug').value=row.slug;link(row.slug)}
 ...
 content.querySelector('#cloud-publish').onclick=run(async()=>{const input=content.querySelector('#publish-slug');if(!input.value||!input.reportValidity())return;if(!await confirmPublish())return;const revision=await saving.flush();const slug=check(await cloud.rpc('publish_page',{requested_slug:input.value,expected_revision:revision,expected_owner:session.user.id}));link(slug);message('Published. The link now shows this version of your page.');});
