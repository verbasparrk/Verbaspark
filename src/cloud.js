...
export function accountDialog(getState,loadState,saving,confirmPublish=async()=>true){
 ...
 content.innerHTML='<p>Loading account…</p>';
 ...
 const domainButton=document.createElement('button');domainButton.id='cloud-domain';domainButton.textContent='Custom domain';content.querySelector('#cloud-account-data').before(domainButton);
 ...
 domainButton.onclick=async()=>{dialog.close();const {openCustomDomain}=await import('./custom-domain.js');openCustomDomain()};
