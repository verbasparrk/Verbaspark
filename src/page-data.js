import {visualThemes} from './visual-themes.js';
import {cleanShowcaseCover} from './showcase-cover.js';
import {cleanProfile,cleanTranslations} from './profile-data.js';
...
export function cleanPage(value){
 ...
 const ids=new Set();const cardId=(card,index)=>{let id=typeof card.id==='string'&&/^[a-zA-Z0-9_-]{1,100}$/.test(card.id)?card.id:'loaded-'+index;while(ids.has(id))id+='-copy';ids.add(id);return id};
