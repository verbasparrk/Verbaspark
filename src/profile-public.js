...
export function profileExtras(page,esc,language,includeForm=true,allowReport=true){const p=cleanProfile(page.profile),codes=[...new Set([p.defaultLanguage,...p.languages])],t=words[language]||words.en;return `${codes.length>1?`<label class="public-language">Language / Jezik<select data-profile-language>${codes.map(code=>`<option value="${code}" ${code===language?'selected':''}>${languages[code]}</option>`).join('')}</select></label>`:''}${p.vcard.enabled?`<a class="vcard-download" download="contact.vcf" href="data:text/vcard;charset=utf-8,${esc(encodeURIComponent(makeVCard({...p.vcard,name:p.vcard.name||page.name})))}">${t.save} ↓</a>`:''}${includeForm?contactFormHTML(page,esc,language):''}${allowReport?`<button class="profile-report-button" type="button" data-profile-report>${language==='sl'?'Prijavi profil':'Report profile'}</button>`:''}`}
...
export function bindPublicProfile(page,{slug='',language='en',changeLanguage,trackView=true}){
 ...
 root.querySelector('[data-profile-report]')?.addEventListener('click',()=>{
  if(!slug){window.alert(t.preview);return}
  const dialog=document.createElement('dialog');dialog.className='profile-report-dialog';
  dialog.innerHTML='<form><h2>Report this profile</h2><p>Tell Verbaspark what needs review. Reports are private and do not automatically hide a page.</p><label>Reason<select name="reason" required><option value="">Choose a reason</option><option value="spam">Spam or scam</option><option value="impersonation">Impersonation</option><option value="harassment">Harassment</option><option value="unsafe">Unsafe content</option><option value="other">Other</option></select></label><label>Details (optional)<textarea name="details" maxlength="2000" rows="4" placeholder="What should we look at?"></textarea></label><div class="form-trap" aria-hidden="true"><label>Website<input name="website" tabindex="-1" autocomplete="off"></label></div><p role="status"></p><div class="report-actions"><button type="button" data-cancel>Cancel</button><button type="submit">Send report</button></div></form>';
  ...
  dialog.querySelector('form').onsubmit=async event=>{event.preventDefault();const form=event.currentTarget,button=form.querySelector('[type=submit]'),status=form.querySelector('[role=status]');button.disabled=true;status.textContent='Sending…';try{const values=Object.fromEntries(new FormData(form));const response=await fetch('/api/report',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...values,slug}),signal:AbortSignal.timeout(20000)});const result=await response.json();if(!response.ok)throw Error(result.error||'Could not send report.');status.textContent='Report received. Thank you.';button.hidden=true}catch(error){status.textContent=error.message;button.disabled=false}};
 });
