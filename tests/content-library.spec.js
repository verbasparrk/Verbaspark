...
test('grouped library chooses useful defaults and works on phones',async({page})=>{
 await page.goto('/editor/');await page.locator('[data-tab="blocks"]').click();await expect(page.getByRole('dialog',{name:'Add content',exact:true})).toBeVisible();await expect(page.locator('.content-group')).toHaveCount(5);await expect(page.locator('.choice-example')).toHaveCount(13);
...
test('duplicate is independent; hidden blocks stay in draft and out of preview and export',async({page})=>{
 await page.goto('/editor/');await page.locator('#duplicate-block').click();await expect(page.locator('#card-title')).toHaveValue('Interface systems (copy)');await expect(page.locator('.card.project')).toHaveCount(2);
