...
test('review lists missing content, opens the exact block and refreshes after a fix',async({page})=>{
 await page.goto('/editor/');await page.locator('[data-tab="blocks"]').click();await page.locator('[data-add="document"]').click();await page.locator('#publish-entry').click();
...
test('mobile review can open settings and ignores unfinished hidden blocks',async({page})=>{
 await page.setViewportSize({width:390,height:844});await page.goto('/editor/');await page.locator('[data-mobile-tool="blocks"]').click();await page.locator('[data-add="gallery"]').click();await page.locator('.sheet-done').click();
