...
test('unfinished typing is recovered after reload and backups can be restored',async({page})=>{
 await page.goto('/');await page.locator('#card-title').fill('Typed without blurring');
...
test('when browser storage fails, edits stay visible and a backup remains downloadable',async({page})=>{
 await page.goto('/');await expect(page.locator('#save-status')).toContainText('Saved on device');
...
test('a damaged newest checkpoint falls back to a valid recovery version',async({page})=>{
 await page.goto('/');await page.locator('#card-title').fill('Recoverable title');await page.locator('#card-title').press('Tab');await expect(page.locator('#save-status')).toContainText('Saved on device');
