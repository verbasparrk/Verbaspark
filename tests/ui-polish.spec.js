...
test('link presentation choices keep accessible destinations and export their style',async({page})=>{
 await page.goto('/');await page.locator('[data-id="github"]').click();await page.locator('[data-link-style="icon"]').click();
...
test('first steps reflect actual edits and disappear after completion',async({page})=>{
 await page.goto('/');await page.locator('[data-first-step="name"]').click();await page.locator('#card-title').fill('My own page');await page.locator('#card-title').press('Tab');
