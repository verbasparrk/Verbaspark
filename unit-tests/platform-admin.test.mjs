...
test('platform moderation is private, audited and cannot be bypassed by republishing',async()=>{
 ...
 try{
  await db.exec(`create role anon;create role authenticated;create role service_role bypassrls;
   ...
   create table public.published_pages(owner_id uuid primary key references auth.users(id),slug text unique,document jsonb,published_at timestamptz default now());
