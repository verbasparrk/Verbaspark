...
test('public endpoints validate, limit, exclude owner analytics and render initial share metadata',async()=>{
 const savedEnv={...process.env},originalFetch=globalThis.fetch,requests=[];let limited=false,contactOn=true,hidden=false;
 ...
 try{
  ...
  hidden=true;
  res=response();await contact(req({slug:'alice',name:'Guest',email:'guest@example.com',message:'Hi'}),res);assert.equal(res.code,404);
  res=response();await event(req({slug:'alice',event:'click',card:'intro'}),res);assert.equal(res.code,404);
  res=response();await page({method:'GET',query:{slug:'alice'},profileShell:'<html lang="en"><head></head><body><div id="app"></div></body></html>'},res);assert.equal(res.code,404);
  hidden=false;
