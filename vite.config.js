...
export default defineConfig(({mode})=>{
 ...
 for(const key of ['SUPABASE_URL','SUPABASE_SERVICE_ROLE_KEY','ANALYTICS_HASH_SECRET','APP_URL','VERCEL_API_TOKEN','VERCEL_PROJECT_ID','VERCEL_TEAM_ID'])if(env[key])process.env[key]=env[key];
 ...
 return {build:{rollupOptions:{input:{home:resolve(import.meta.dirname,'index.html'),editor:resolve(import.meta.dirname,'editor/index.html'),admin:resolve(import.meta.dirname,'admin/index.html')}}},plugins:[{name:'search-console-verification',transformIndexHtml(html,context){if(!['/','/index.html'].includes(context.path)||!verification)return html;if(!/^[A-Za-z0-9_-]{10,200}$/.test(verification))throw Error('Invalid GOOGLE_SITE_VERIFICATION value.');return html.replace('</head>',`<meta name="google-site-verification" content="${verification}"></head>`)}},{name:'local-profile-services',configureServer(server){server.middlewares.use(async(req,res,next)=>{
  ...
  const endpoint=url.pathname==='/api/report'?'reports':['/api/account','/api/admin','/api/contact','/api/event','/api/cover','/api/image','/api/sitemap','/api/storage','/api/reports','/api/domain'].includes(url.pathname)?url.pathname.slice(5):['/sitemap.xml','/robots.txt'].includes(url.pathname)?'sitemap':slug&&process.env.APP_URL?'page':null;
  ...
  try{
   ...
   req.query={...Object.fromEntries(url.searchParams),...(slug?{slug}:{}),...(url.pathname==='/api/report'?{public:'1'}:{}),...(url.pathname==='/robots.txt'?{format:'robots'}:{})};res.status=code=>{res.statusCode=code;return res};res.json=value=>{res.setHeader('Content-Type','application/json');res.end(JSON.stringify(value))};res.send=value=>res.end(value);
